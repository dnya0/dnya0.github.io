...
| StatefulSet | 상태를 가진 애플리케이션을 관리하는 리소스 |
| DaemonSet | 모든 Node 또는 특정 Node마다 Pod를 실행하는 리소스 |
...
Pod 자체는 컨테이너가 실행되는 공간에 가깝고, 복구나 업데이트 같은 운영 기능은 제한적이기 때문이다.

예를 들어 단순히 Pod만 생성하면 아래와 같은 상황을 직접 관리해야 한다.
...
정리하면 Pod는 실제 실행 단위이고, Deployment는 Pod를 운영하기 위한 상위 관리 리소스다.
...
StatefulSet은 상태를 가진 애플리케이션을 관리할 때 사용하는 리소스다.
Deployment가 Stateless 애플리케이션에 주로 사용된다면, StatefulSet은 Pod의 순서와 고정된 식별자가 중요한 애플리케이션에 사용된다.

예를 들어 PostgreSQL, Kafka, Elasticsearch처럼 저장 데이터나 노드 식별이 중요한 경우를 생각할 수 있다.

StatefulSet은 Pod 이름을 예측 가능한 형태로 유지한다.
...
또한 각 Pod에 안정적인 네트워크 식별자와 storage를 연결할 수 있다.
그래서 단순히 같은 컨테이너 여러 개를 띄우는 Deployment와는 목적이 다르다.

<br>

### DaemonSet

DaemonSet은 모든 Node 또는 특정 조건의 Node마다 Pod를 하나씩 실행할 때 사용하는 리소스다.

대표적인 사용 사례는 로그 수집기나 모니터링 에이전트다.
각 Node에서 로그를 수집하거나 메트릭을 모으려면 Node마다 에이전트가 떠 있어야 한다.
...
새로운 Node가 클러스터에 추가되면 DaemonSet은 해당 Node에도 자동으로 Pod를 생성한다.

<br>

### 운영 흐름 기준으로 보기

리소스를 운영 관점에서 보면 대략 아래처럼 나눌 수 있다.

| 상황 | 주로 사용하는 리소스 |
| :--- | :--- |
| Web/API 서버처럼 상태가 없는 애플리케이션 배포 | Deployment |
| PostgreSQL, Kafka처럼 상태가 중요한 애플리케이션 배포 | StatefulSet |
| 로그 수집기, 모니터링 에이전트처럼 모든 Node에 필요한 프로그램 배포 | DaemonSet |
